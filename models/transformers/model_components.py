...
from nnunet.network_architecture.transformers.layers import LayerNorm
from nnunet.network_architecture.transformers.transformerblock import TransformerBlock
from nnunet.network_architecture.transformers.dynunet_block import get_conv_layer, UnetResBlock


einops, _ = optional_import("einops")

class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4 , 2 * 2 * 2 ],dims=[32, 64, 128, 256, 512],
                 proj_size =[64,64,64,64,32], depths=[3, 3, 3, 3],  num_heads=4, spatial_dims=3, in_channels=1, dropout=0.0, transformer_dropout_rate=0.0 ,**kwargs):
        super().__init__()
    ...
    def forward_features(self, x):
        ...
        for i in range(1, 5):
            ...
            if i == 4:  # Reshape the output of the last stage
                #print('b c h w d: ', x.shape)
                x = einops.rearrange(x, "b c h w d -> b (h w d) c")
                #print('b(hwd)c: ', x.shape)
            hidden_states.append(x)
        #print('len(hidden_states): ', len(hidden_states), hidden_states[-1].shape)
        #for i in range(len(hidden_states)):
            #print('hidden_states: ', hidden_states[i].shape)
...
class UnetrPPEncoder_full(nn.Module):
    def __init__(self, input_size=[64 * 64 * 64, 32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4  ],dims=[32, 64, 128, 256, 512],
                 proj_size =[64,64,64,64,32], depths=[3, 3, 3, 3],  num_heads=4, spatial_dims=3, in_channels=1, dropout=0.0, transformer_dropout_rate=0.0 ,**kwargs):
        super().__init__()
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(TransformerBlock(input_size=out_size, hidden_size= out_channels, proj_size=proj_size, num_heads=num_heads,
                                                     dropout_rate=0.0, pos_embed=True))
