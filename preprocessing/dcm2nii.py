...
def convert_dicom_to_nifty(input_filepaths,
    ...
    slices = [pdcm.read_file(dcm) for dcm in input_filepaths if hasattr(pdcm.read_file(dcm), 'ImagePositionPatient')]
    print(len(slices))
    slices.sort(key=lambda x: float(x.ImagePositionPatient[2]))
    ...
    output_filepath = join(output_folder,patient_id+'__'+modality+extension)
    ...
    if rtstruct_file is not None:
        masks = get_masks(rtstruct_file,
                        ...
                        dtype=dtype_image)
        if combine_labels:
            combined_mask = np.zeros_like(np_image, dtype=np.int16)  # Initialize an empty mask
            for i, (label, np_mask) in enumerate(masks):
                # Add the mask to the combined mask. 
                # Assign a different integer value to each label.
                combined_mask[np_mask > 0] = i+1
            
            #output_filepath_mask = join(output_folder, patient_id + '_combined' + extension)
            output_filepath_mask = join(output_folder, patient_id + '__gtv' + extension)

            sitk_mask = get_sitk_volume_from_np(combined_mask, pixel_spacing,
                                                image_position_patient)
            
            sitk.WriteImage(sitk_mask, output_filepath_mask)

        else:
            for label, np_mask in masks:
                # output_filepath_mask = output_filepath.split(
                #     '.')[0] + '_' + label.lower() + extension
                output_filepath_mask = join(output_folder,patient_id+'__'+label.lower()+extension)
...
def get_best_match(target, options, threshold=80):
    """
    Find the best fuzzy match for the target in the options.
    ...
    Returns:
        str: The best match, or None if no match above the threshold was found.
    """
    best_match = None
    best_ratio = 0

    for option in options:
        ratio = fuzz.ratio(target.lower(), option.lower())
        if ratio > best_ratio:
            best_ratio = ratio
            best_match = option
...
#         for label in labels:
#             fuzz_match = get_best_match(label, roi_seq.ROIName)
#             if fuzz_match is not None:
...
def read_structure(rtstruct_file, labels=['GTVt', 'GTVn']):
    structure = pdcm.read_file(rtstruct_file)
    contours = []
    roi_names = [roi_seq.ROIName for roi_seq in structure.StructureSetROISequence]
    
    for label in labels:
        match_name = get_best_match(label, roi_names)
        print(match_name)
        if match_name is not None:
            for i, roi_seq in enumerate(structure.StructureSetROISequence):
                if roi_seq.ROIName == match_name:
                    contour = {}
                    contour['color'] = structure.ROIContourSequence[i].ROIDisplayColor
                    contour['number'] = structure.ROIContourSequence[i].ReferencedROINumber
                    contour['name'] = label #roi_seq.ROIName
                    assert contour['number'] == roi_seq.ROINumber
                    contour['contours'] = [
                        s.ContourData
                        for s in structure.ROIContourSequence[i].ContourSequence
...
if __name__ == '__main__':
    image_folder = '/mnt/data/shared/hecktor2022/KM_Forskning_01/2021-01__Studies/fsdWvHtaAYEqcuDC_qiZd3pdfha5HHK4r0H15oQA44_CT_2021-01-28_170228_._._n136__00000'
    rt_strcut_folder = '/mnt/data/shared/hecktor2022/KM_Forskning_01/2021-01__Studies/fsdWvHtaAYEqcuDC_qiZd3pdfha5HHK4r0H15oQA44_RTst_2021-01-28_170228_._Contouring_n1__00000'
    input_filepaths = glob.glob(os.path.join(image_folder, '*'))
    rt_files = glob.glob(os.path.join(rt_strcut_folder, '*'))
    #input_filepaths= input_filepaths + rt_files
    print(input_filepaths)

    patient_id = 'KM_Forskning_01'

    output_folder = '/mnt/data/shared/hecktor2022/KM_Forskning_nii'
    convert_dicom_to_nifty(input_filepaths,
                           output_folder,
    ...
    image_folder = '/mnt/data/shared/hecktor2022/KM_Forskning_02/2020-07__Studies/YXIeldCqxgYxvEhw_q1BE3YPSYPpjfB39yWaYcR06N_CT_2020-07-07_134341_._._n140__00000'
    rt_strcut_folder = '/mnt/data/shared/hecktor2022/KM_Forskning_02/2020-07__Studies/YXIeldCqxgYxvEhw_q1BE3YPSYPpjfB39yWaYcR06N_RTst_2020-07-07_134341_._Contouring_n1__00000'
    ...
    convert_dicom_to_nifty(input_filepaths,
                           ...
                           dtype_image=np.float32,
                           dtype_mask=np.uint8)
    image_folder = '/mnt/data/shared/hecktor2022/KM_Forskning_03/2021-01__Studies/mdfOijiVMHfiScYa_81igm3vISlnzEN8hjg8Ydogr9_CT_2021-01-28_174114_._._n151__00000'
    rt_strcut_folder = '/mnt/data/shared/hecktor2022/KM_Forskning_03/2021-01__Studies/mdfOijiVMHfiScYa_81igm3vISlnzEN8hjg8Ydogr9_RTst_2021-01-28_174114_._Contouring_n1__00000'
